...
function certKeyThumbprint(certDERBuf) {
  let certDER = forge.util.hexToBytes(certDERBuf.toString('hex'));
  let cert = forge.pki.certificateFromAsn1(forge.asn1.fromDer(certDER));
  let certSPKIBytes = forge.asn1.toDer(forge.pki.publicKeyToAsn1(cert.publicKey));
  let certSPKI = new Buffer(forge.util.bytesToHex(certSPKIBytes), 'hex');
  return jose.JWK.asKey(certSPKI, 'spki')
