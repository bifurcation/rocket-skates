...
//
// Doing OOB like this (using the same express app) makes it easy to do basic
// things and easy to test.  But probably imposes some limitations on the types
// of OOB things you can do.
class OutOfBandRequirement {
  constructor(server, regID, handler) {
    this.id = uuid.v4();
    this.regID = regID;
    this.status = 'pending';
    this.url = server.makeURL(this);
  ...
  handle(req, res) {
    return this.handler(req, res)
      .then(result => {
        this.status = (result)? 'valid' : 'invalid';
      })
      .catch(() => {
        this.status = 'invalid';
      })
      .finally(() => {
        this.server.db.updateAppsFor(this);
      });
  }

  type() {
    return OutOfBandRequirement.type;
  }
  ...
  updateAppsFor(reqIn) {
    ...
    for (let key in this.store[Application.type]) {
      if (this.store[Application.type].hasOwnProperty(key)) {
        ...
        if (app.regID !== reqIn.regID) {
          continue;
        }

        app.requirements.map(req => {
          if (req.url === reqIn.url) {
...
class ACMEServer {
  constructor(options) {
    ...
    // Import out-of-band requirements from caller preferences
    this.oobHandlers = [];
    if (options.oobHandlers) {
      this.oobHandlers = options.oobHandlers;
    }
  ...
  fetch(req, res) {
    ...
    // Branch for OOB
    if (type === OutOfBandRequirement.type) {
  ...
  newApp(req, res) {
    ...
    // Add any specified out-of-band requirements
    for (let oobHandler of this.oobHandlers) {
      let oobReq = new OutOfBandRequirement(this, reg.id, oobHandler);
      this.db.put(oobReq);
      app.requirements.push(oobReq.asRequirement());
    }
  ...
  keyChange(req, res) {
    ...
    jose.verify(req.payload)
      ...
      .then(tpBuf => {
        ...
        let msg = (!urlMatch)? `URL doesn not match ${inner.header.url} ${outer.header.url}` :
                  (!oldKeyMatch)? `old key does not match ${inner.payload.oldKey} ${innerThumbprint}` :
                  (!newKeyMatch)? `new key does not match ${inner.payload.newKey} ${outer.accountKeyThumbprint}` :
                  (!regMatch)? `reg does not match ${inner.payload.account} ${reg.url}` :
                  null;

        if (!urlMatch || !oldKeyMatch || !newKeyMatch || !regMatch) {
          res.status(403);
          res.send(problem('unauthorized', 'Could not verify key-change request: ' + msg));
          res.end();
          return;
        }

        reg.key = outer.accountKey;
        reg.thumbprint = outer.accountKeyThumbprint;
        this.db.put(reg);
