    ...
    return new Promise(resolve => {
      server.on('request', (req, res) => {
        if (req.question.length === 0) {
          res.header.rcode = dns.consts.NAME_TO_RCODE.FORMERR;
        } else if ((req.question[0].class !== dns.consts.NAME_TO_QCLASS.IN) ||
            (req.question[0].type !== dns.consts.NAME_TO_QTYPE.TXT) ||
            (req.question[0].name !== recordName)) {
